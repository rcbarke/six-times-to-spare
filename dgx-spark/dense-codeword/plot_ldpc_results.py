...
def plot_throughput_vs_iter(df: pd.DataFrame, out_path: str) -> None:
    ...
    plt.title("LDPC5G throughput vs iterations on DGX Spark (N_cw >= 2048)")
...
def plot_throughput_vs_codewords(df: pd.DataFrame, out_path: str) -> None:
    """
    Plot average CPU vs GPU throughput as a function of number of codewords (N_cw).
    """
    agg = (
    ...
    plt.ylabel("Throughput [Mbit/s]")
    plt.title("LDPC5G throughput vs N_cw on DGX Spark (N_cw >= 2048)")
    plt.grid(True, which="both", axis="both")
    plt.legend()
...
def plot_resource_utilization(
    ...
    axes[0].set_title("CPU utilization during LDPC sweep (N_cw >= 2048)")
...
def main() -> None:
    ...
    # Figure 2: throughput vs codewords
